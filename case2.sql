...
WITH Pizza AS(
    SELECT c.order_id, COUNT(c.pizza_id) AS Pizza_Ordered
    FROM customer_orders as c
...
SUM(
    CASE WHEN c.extras = '' OR c.exclusions = '' THEN 1
    ELSE 0
    END) AS One_Change
SUM(
    CASE WHEN c.extras = '' AND c.exclusions = '' THEN 1
    ELSE 0
    END) AS No_Change
FROM customer_orders as c 
INNER JOIN runner_orders as r ON c.order_id = r.order_id
